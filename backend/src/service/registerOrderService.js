...
async function rejectOrder(idCollector) {
  const connection = await mysql.createConnection(databaseConfig);

  const [recuser] = await connection.query(
  `SELECT * FROM registerOrder
   WHERE status != 1
   AND (idCollector IS NULL OR idCollector = ?)
   AND NOT EXISTS (
      SELECT 1 FROM orderRejections
      WHERE idRegisterOrder = registerOrder.idRegisterOrder
      AND idCollector = ?
   );`,
    [idCollector, idCollector] // <<< dois valores!
  );
